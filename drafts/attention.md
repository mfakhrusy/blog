...
Right, the semantic meaning in conversation is easy enough to understand. So, what does it has to do with LLM, specifically, what makes this word special enough to become the first word of the transformer paper—Attention is all you need? Well, one way to find out.
...
Interesting! The general idea of that paper is the implementation of attentional mechanism to translate text using RNN. The paper introducs two general architecture: Global and Local attention.
...
`seq2seq` model is using an <span class="inline-tooltip" tabindex="0"><a href="https://www.tensorflow.org/text/tutorials/nmt_with_attention#the_encoderdecoder" target="_blank" rel="noopener noreferrer">encoder-decoder</a><sup>?</sup><span class="tooltip-bubble"><img src="../assets/attention/nmt-encoder-decoder.png"></span></span> approach; the blue ones are there to express the encoding phase, while the red ones are for expressing the decoding phase. Figure 2 and 3 represents the global and local approach of the attention mechanism for translating text respectively. The global/local difference is akin to when I open my eyes whole vs when I squint my eyes to focus on to something. If I want to have a full picture of my surrounding, I open my eyes—my sense—to feel the whole surrounding area. If I want to focus on a specific part of the environment, I would use my eyes to stare at that point and my brain will automatically filter unimporant informations unrelated to the thing that I want to focus on. I don't think the metaphor perfectly encapsulate the meaning that the author intends to convey, so I'd like to quote the explanation—which I'm happy to say, is a well-articulated one:
...
I like this explanation a lot. If I may rephrase, I'd say global attention approach of a translation function is when the whole paragraph, the whole chunk of texts—can even be the whole book where the word is sourced from—are considered to get its semantic meaning while local attention is much more focused to just a few neighboring words. Intuitively, I can also say the the global attention function calculation is much more expensive than the local ones; basically a trade-off. 

Now, I want to see if I can apply this intuitive understanding of Attention to something that is more familiar: AI-assisted programming. For this example, I would initiate a conversation to a chatbot that uses LLM—transformer—as its main driver. Let's try making a simple python script to handle a POST request with this query:

`"create a python post request handler of a login form"`

A very simple and straightforward query (or prompt, as many people are saying). The response is available <span class="inline-tooltip" tabindex="0"><a href="https://gist.github.com/mfakhrusy/3134f3bf40f7b6eedd1b488e65d4fb11" target="_blank" rel="noopener noreferrer">in my public github gist</a><sup>?</sup><span class="tooltip-bubble"><img src="../assets/attention/kimi-response-excerpt-1.png"></span></span>. The result makes sense at first, well maybe a little bit weird because I asked for a simple backend code, but the chatbot involutarily added a security-related text in their responses. I didn't ask for that, I don't even know whether the code is actually secure without actually looking at it. So I have to actually read the resulted code and check... by injecting it to another chatbot with an added bonus: An explicitly security-related query. The query is simple, I would write, `how secure is this code` plus I would copy all of the generated code—and only the code—to a new session of the chatbot.

The result is saved in <span class="inline-tooltip" tabindex="0"><a href="https://gist.github.com/mfakhrusy/df202afe19f25efcaf3a2fb726b0ed99" target="_blank" rel="noopener noreferrer">my other public github gist</a><sup>?</sup><span class="tooltip-bubble"><img src="../assets/attention/kimi-response-excerpt-2.png"></span></span>. Next, I'd like to analyze the results of the second generated text and see if the first generated text—the initial python script—is implemented with, in its words, "security best practices."

First, the second generated text complaint about the usage of SHA256 for password hashing. I agree, `argon2` or `bcrypt` is a standard practice for hashing a password due to its slowness—Yes, <span class="inline-tooltip" tabindex="0"><a href="https://security.stackexchange.com/questions/150620/what-is-the-purpose-of-slowing-down-the-calculation-of-a-password-hash" target="_blank" rel="noopener noreferrer">password hash computation need to be computationally slow</a><sup>?</sup><span class="tooltip-bubble"><img src="../assets/attention/password-hash-slow.png"></span></span>. Next, timing attack. Frankly, this is a <span class="inline-tooltip" tabindex="0">TIL<sup>?</sup><span class="tooltip-bubble">Today I learned</span></span> moment for me. I knew about the existence of timing-based attacks, but I couldn't quite understand it until I see this example:

```python
return hash_obj.hexdigest() == stored_hash
```

The above code problematic because `hexdigest` will return as soon as it finds a mismatch, so, theoritically, an attacker can enumerate a password hash by guessing and see its timing result. The last critical vulnerabilities is a hardcoded demo credentials, let's see the actual generated code from the first query:
...
In my second query, I added a security-related text, and the resulted text is focused on the initial code's security aspect. People are saying this is "prompt engineering", and yes, in a way, it is, but I would also say this is a direct implementation on how Attention work in practice. The Attention mechanism of a transformer helped the resulting text focus on the security-related part of the query.
